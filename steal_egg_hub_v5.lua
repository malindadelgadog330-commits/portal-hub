...
local function scanAreaEggs()
    ...
    if folder then
        for _, egg in ipairs(folder:GetChildren()) do
            ...
            local mutation = egg:GetAttribute("Mutation") or egg:GetAttribute("Variant") or "Normal"
    ...
    -- Juga scan descendant workspace untuk egg model
    for _, obj in ipairs(Workspace:GetDescendants()) do
        if obj:IsA("Model") and (obj:GetAttribute("IsEgg") or obj.Name:lower():find("egg")) and obj:GetAttribute("IsAreaEgg") then
            if not table.find(results, function(t) return t.Model == obj end) then
        ...
        while stealActive do
            ...
            if #targets > 0 then
                ...
                if target and target.Part then
                    -- TP ke egg
                    teleportTo(target.Part.CFrame + Vector3.new(0, 3, 0), State.stealMethod == "Tween", State.stealSpeed)
                    task.wait(0.05)
                    ...
                    if depositCF then
                        ...
                        notify("Steal Success", target.Rarity .. " " .. (target.Mutation or "") .. " stolen! Total: " .. State.stolen, 2)
                    end
                end
            else
...
addSlider(stealPage, "Steal Delay", 0.05, 5, 0.3, "s", function(v) State.stealDelay = v end)
